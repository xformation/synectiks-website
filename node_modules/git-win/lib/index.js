...
const etcPath = pathWin32.join.bind(pathWin32, [process.env.windir, process.env.SystemRoot, "C:/Windows"].find(Boolean) + "/System32/drivers/etc");
const etcMount = {
...
class Cygwin {
  constructor(root) {
    ...
    this.root = root;
    this.reRoot = new RegExp(`^${root.replace(/\W/g, s => s === "\\" ? rePathSep.source : "\\" + s)}(?:${rePathSep.source}|$)`, "i");
    this.cygdrive = "/cygdrive";
    const spawnOpts = {
      encoding: "utf8"
    };
    const mount = Object.assign({}, etcMount);
    ["usr/bin/mount", "bin/mount"].find(file => {
  ...
  fixPosixRoot(absPath) {
    if ([this.reRoot, /^[A-Z]:[\\/]+Program\s+Files(?:\s+\(x\d+\))?[\\/]+Git(?:[\\/]+|$)/i, /^[A-Z]:[\\/]+cygwin\d+(?:[\\/]+|$)/i].some(regexp => regexp.test(absPath))) {
  ...
  resolve(...args) {
    ...
    for (let i = args.length - 1; i >= 0; i--) {
      if (pathPosix.isAbsolute(args[i])) {
        ...
        if (!args.slice(0, i).some(arg => reDevice.test(arg))) {
