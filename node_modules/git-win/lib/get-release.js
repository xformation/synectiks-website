...
function updateCache(_x) {
  return _updateCache.apply(this, arguments);
}

function _updateCache() {
  _updateCache = (0, _asyncToGenerator2.default)(function* (data) {
    cache = cache.filter(oldRelease => !data.some(newRelease => newRelease.id === oldRelease.id));
    ...
    yield fs.writeFile(path.join(__dirname, "data-cache.json"), JSON.stringify(cache, null, "\t") + "\n");
    return cache;
  });
  return _updateCache.apply(this, arguments);
}
...
function _getData() {
  _getData = (0, _asyncToGenerator2.default)(function* (url) {
    // http请求数据
    try {
...
function _getReleases() {
  _getReleases = (0, _asyncToGenerator2.default)(function* () {
    const releases = yield getData("https://api.github.com/repos/git-for-windows/git/releases");
    return releases;
  });
  return _getReleases.apply(this, arguments);
}
...
function getRelease(_x3) {
  return _getRelease.apply(this, arguments);
}

function _getRelease() {
  _getRelease = (0, _asyncToGenerator2.default)(function* (version) {
    const releases = yield getReleases();

    if (version) {
      return releases.find(release => release.tag_name.indexOf(version) === 1);
    } else {
      return releases[0];
    }
  });
