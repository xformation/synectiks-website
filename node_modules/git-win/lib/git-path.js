...
function getGitDirByRegstry(arch) {
  const args = ["QUERY", "HKLM\\SOFTWARE\\GitForWindows"];
...
function envPathResolve(strPath) {
  return strPath.replace(reEnvKey, (s, envKey) => {
...
function findGitDir(dirs) {
  ...
  return result.find(dir => findFile(dir, "cmd/git.exe"));
}
...
function lookupGitDir(dirs = [// by install default
"%ProgramW6432%/Git", "%ProgramFiles%/Git", // by install x32 under win x64 default
"%ProgramFiles(x86)%/Git", // by install default with out Admin
"%APPDATA%/Programs/Git", // by SourceTree
"%APPDATA%/Atlassian/SourceTree/git_local", // by Cmder
"%GIT_INSTALL_ROOT%", // by Cmder
...
function getGitDirByPathEnv(PATH = process.env.PATH) {
  const dirs = PATH.split(path.delimiter).map(dir => dir && (/^(.*[\\/]Git)(?=[\\/]|$)/i.test(dir) || /^(.+?)[\\/]+(?:cmd|(?:usr|mingw\d+)[\\/]+bin)(?=[\\/]|$)/i.test(dir)) && RegExp.$1);
  return findGitDir(dirs);
}
...
function getMingwDir(gitDir) {
  ...
  let result = ["64", "32"].filter(arch => arch !== osArch);
